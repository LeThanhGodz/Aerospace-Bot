...
//checked
  run: async function (client, message, args) {
    ...
    if(message.guild.me.voice.channel !== channel)return sendError(`${process.env.EMOTE_NO || '❌'}`+' | The bot is already in an another channel, you need to join voice channel where the bot is to use this command!', message, client);
  }
    await channel.join();
    
    message.react(process.env.EMOTE_OK.replace(/<(a):([^+]*)([A-Za-z0-9]*)([^+]*)([A-Za-z0-9]*):/g, "").replace(/>/g, "")||"✅");
    sendSuccess(`${process.env.EMOTE_OK || '✅'}`+" | Joined Successfully!", message);
  ...
  interaction: async function (client, message, args){//message=interaction
   ...
   let channel=await client.guilds.cache.get(message.guild_id).members.cache.get(message.member.user.id).voice.channel
    ...
    if (client.guilds.cache.get(message.guild_id).me.voice.channel !== channel)return sendError(`${process.env.EMOTE_NO || '❌'}`+' | The bot is already in an another channel, you need to join voice channel where the bot is to use this command!', message, client);
  }
    await channel.join();
    //message.react(process.env.EMOTE_OK.replace(/<(a):([^+]*)([A-Za-z0-9]*)([^+]*)([A-Za-z0-9]*):/g, "").replace(/>/g, "")||"869920204786925608");
    sendSuccess(`${process.env.EMOTE_OK || '❌'}`+" | Joined Successfully!", message, client);
