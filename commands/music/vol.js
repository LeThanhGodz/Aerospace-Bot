...
//checked
  run: async function (client, message, args) {
    ...
    if (message.guild.me.voice.channel.id !== channel.id)return sendError(`${process.env.EMOTE_NO || '❌'}`+' | You need to join voice channel where the bot is to use this command!', message);
    ...
    .setDescription(`${process.env.EMOTE_OK || '✅'} | I set the volume to: **${args[0]}/100**`)
...
if(arg)args=[arg.find(arg => arg.name.toLowerCase() == "volume").value]  
    ...
    if (!serverQueue) return sendError(`${process.env.EMOTE_NO || '❌'}`+" | There is nothing playing in this server.", message, bot);
