...
exports.run = (bot, message, args) => {
    ...
    let perm=message.channel.permissionsFor(message.member)//perm.has()
      ...
      if(isNaN(args[0].replace("m", "").replace("d", "").replace("ms", "").replace("h", "").replace("s", ""))){
        return message.mentionReply(`${process.env.EMOTE_NO || '❌'} | please give nummerical numbers or <number>ms/<number>s/<number>m/<number>h/<number>d`)
      }
      bot.db.set(`${message.guild.id}_mutetime`, require("ms")(args[0]))
      
      message.noMentionReply(
        `${process.env.EMOTE_OK || '✅'} | Successfully setup mute time`
      );
      return;
}
